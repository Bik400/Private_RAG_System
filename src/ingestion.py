...
def delete_index(client: OpenSearch) -> None:
    """
    Deletes an index in OpenSearch if it exists
...
def bulk_index_documents(documents: List[Dict[str, any]]) -> Tuple[int, List[any]]:
    ...
    Returns:
        Tuple[int,List[any]] -> Tuple with the number of successfully indexed documents and a list of errors
    """
    ...
    for doc in documents:
        ...
        # Prefix each document's text with "passage: " fro the asymmetric embedding model
        ...
        # Perform bulk indexing and capture response details 
        success, errors = helpers.bulk(client, actions)
        logger.info(f"Bulk indexed {len(documents)} documents into index {OPENSEARCH_INDEX} with {len(errors)} errors.")
...
def delete_documents_by_document_name(document_name: str) -> Dict[str,any]:
    ...
    response: Dict[str, Any] = client.delete_by_query(index=OPENSEARCH_INDEX, body=query)
