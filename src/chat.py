...
def ensure_model_pulled(model: str) -> bool:
    ...
    Returns:
        bool -> True if the model is available else False
    """
    try:
        ...
        if model not in available_models:
            ...
            ollama.pull(model)
            logging.info(f"Model {model} has been pulled and is now available locally.")
        else:
...
def run_llama_streaming(prompt: str, temperature: float) -> Optional[Iterable[str]]:
    ...
    Returns:
        Optional[Iterable[str]] -> A generator yielding response chunks as strings, or None if an error occurred
    """

    try:
        # Attemp to stream the response from the model
...
def prompt_template(query: str, context: str, history: List[Dict[str, str]]) -> str:
    """
    Build the prompt with context, conversation history, and user query
    ...
    if context:
        ...
        prompt += "Answer the questions to the best of your knowledge.\n"
...
def generate_response_streaming(
    ...
    """
    Generate a chatbot response by performing hybrid search and incorporating history.

    Args:
        query (str) -> The user's query
        use_hybrid_seach (bool) -> Whether to use hybrid search for context
    ...
    history = chat_history[-max_history_messages:]      # Get last 10 messages
    context = ""
