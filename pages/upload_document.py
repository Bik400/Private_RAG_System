...
# Set up page config with title, icon, and layout
...
# Sidebar header
st.sidebar.markdown(
    "<h2 style='text-align: center;'>RAG System</h2>",
    unsafe_allow_html=True
)
st.sidebar.markdown(
...
def render_upload_page() -> None:
    ...
    if 'embedding_models_loaded' not in st.session_state:
        ...
        model_loading_placeholder.empty()   # Clear placeholder after loading
    ...
    for name in document_names:
        ...
        if os.path.exists(file_path):
            ...
            st.session_state['documents'].append(
                {'filename': name, 'content':text, 'file_path': file_path}
            )
        else:
            st.session_state['documents'].append(
                {'filename': name, 'content': '', 'file_path': None}
            )
            logger.warning(f"File {name} does not exist locally.")
    
    if 'deleted_file' in st.session_state:
        st.success(
            f"The file '{st.session_state['deleted_file']}' was sucessfully deleted."
        )
    ...
    if uploaded_files:
        with st.spinner('Uploading and processing documents. Please wait...'):
            for file in uploaded_files:
                if file.name in document_names:
                    st.warning(
                        f"The file '{file.name}' already exists in the index."
                    )
                    continue
                ...
                documents_to_index = [
                    {
                        'doc_id': f"{uploaded_files.name}_{i}",
                        'text': chunk, 
                        'embedding': embedding,
                ...
                logger.info(f"File '{file.name}' uploaded and indexed.")

        st.success('Files uploaded and indexed successfully!')
    
    if st.session_state['documents']:
        st.markdown('### Uploaded Dcouments')
