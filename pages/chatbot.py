...
def render_chatbot_page() -> None:
    ...
    # Initialize session state variabels for chat settings
    ...
    st.sidebar.markdown(
        "<h4 sty;e='text-align: center;'>Your Conversational Platform</h4>",
        unsafe_allow_html=True
    )
        ...
        with st.chat_message('assistant'):
            with st.spinner('Generating response...'):
                ...
                response_stream = generate_response_streaming(
                    prompt, 
                    use_hybrid_search=st.session_state['user_hybrid_search'],
                ...
                if response_stream is not None:
                    for chunk in response_stream:
                        if (
                            ...
                            logger.error('Unexpected chunk in format response stream.')
